...
# Para producción, establece el nivel a INFO para que los mensajes DEBUG no se muestren.
...
# Aseguramos que el handler también filtre los mensajes DEBUG si el logger lo hace.
...
# Este filtro se usará en las plantillas Jinja2 para formatear números como moneda.
@app.template_filter('euro_format')
def euro_format_filter(value):
    try:
        # Asegúrate de que el valor sea numérico, Jinja2 ya debería haberlo convertido a int
        # pero es bueno tener una capa de seguridad.
        decimal_value = Decimal(value)
        # Formatear a moneda usando el locale configurado globalmente.
        return locale.currency(decimal_value, grouping=True, symbol=True)
    except (InvalidOperation, TypeError):
        # En caso de que el valor no sea convertible a Decimal, devuelve un mensaje.
        logger.warning(f"Intento de formatear un valor no numérico '{value}' con euro_format.")
        return "N/A" # O puedes retornar una cadena vacía o un guión.
...
try:
    if CLOUD_STORAGE_BUCKET and os.environ.get('GCP_SERVICE_ACCOUNT_KEY_JSON'):
        ...
        # Mensaje de depuración. No se mostrará con logger.setLevel(logging.INFO).
...
def negocio(empresa_id):
    ...
    try:
        ...
        if empresa:
            ...
            # Mensaje de advertencia. Se mostrará.
