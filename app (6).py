...
def index():
    ...
    sectores_unicos = sorted({s for lst in ACTIVIDADES_Y_SECTORES.values() for s in lst})
    return render_template(
        'index.html',
        empresas=empresas,
        actividades=list(ACTIVIDADES_Y_SECTORES.keys()),
        sectores=sectores_unicos,
...
def publicar():
    ...
    return render_template('vender_empresa.html', actividades=list(ACTIVIDADES_Y_SECTORES.keys())
...
def editar_anuncio(empresa_id):
    ...
    cur.close()
    conn.close()
    return render_template('editar.html', empresa=empresa, actividades=list(ACTIVIDADES_Y_SECTORES.keys())
